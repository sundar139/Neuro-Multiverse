...
`verify.ps1` and `verify.sh` run the same gates: Python version, Ruff format, Ruff lint, mypy, pytest, the runtime metadata smoke test, pre-commit across tracked files, an oversized-file scan, a tracked-artifact scan, a credential scan, a Git diff gate, and a working-tree stability gate. Both print a pass/fail summary and exit nonzero on failure. Neither installs packages or mutates an environment — a validator that changes what it validates cannot be trusted.

The Git diff gate in all four scripts checks the working-tree diff (`git diff --check`) and the staged diff (`git diff --cached --check`) independently, so trailing whitespace or a conflict marker is caught whether or not it has been staged. Staged whitespace that no working-tree check would see is exactly the class of defect this closes.
...
A project `renv` environment is initialized against the Windows R installation. `renv.lock` was generated by `renv` (not hand-authored) and records R 4.6.1 and the `renv` package version actually installed here (1.2.3). The lock is currently bare — it pins `renv` itself and the R version, and the statistical package set is added later when variance-attribution code is written.

`renv` generates `.Rprofile`, `renv/activate.R`, `renv/settings.json`, and `renv/.gitignore`, which are versioned; the `renv` package library, caches, and staging area are not, as `renv/.gitignore` and the repository ignore rules exclude them. The versioned bootstrap and settings files are subject to pre-commit validation like any other tracked file — they are **not** excluded from the hooks. Only the ignored runtime directories (`renv/library/`, `renv/local/`, `renv/cellar/`, `renv/lock/`, `renv/python/`, `renv/sandbox/`, `renv/staging/`) stay outside version control, and `pre-commit --all-files` operates on tracked files only, so no global exclusion is needed for them.

### 7.1 What a valid R environment requires

`scripts/verify_system.ps1` treats the R environment as valid only when all of the following hold:

- `renv::lockfile_read("renv.lock")` succeeds;
- the lock's R version matches the running Windows R runtime;
- the `renv` package record exists in the lock;
- `renv::status(project = ".")` reports a consistent project;
- `renv::lockfile_validate(project = ".", lockfile = "renv.lock", error = TRUE, verbose = TRUE)` returns exactly `TRUE`.

Schema validation is called with **named** `project` and `lockfile` arguments. The positional single-string form `renv::lockfile_validate("renv.lock")` is incorrect: it passes the lockfile path as the `project` argument and does not validate the lockfile schema. The project's `renv` library carries the `jsonvalidate` and `V8` packages that back schema validation; the check activates the project (`renv::load(".")`) so that library is on the path before validating.

### 7.2 Generated-file normalization

Generated files such as `renv/activate.R` may receive non-semantic normalization before commit — line-ending conversion, trailing-whitespace removal, and final-newline fixing — applied by the standard pre-commit hooks. Semantic edits to `renv/activate.R` are prohibited: its R expressions, strings, versions, hashes, and bootstrap logic are not hand-modified. If normalization ever appeared to change behaviour, the environment would be regenerated rather than edited.
