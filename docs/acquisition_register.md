...
- **Raw validation status:** Blocked on raw-file permissions. All 22 acquired files remain hash- and size-identical to the acquisition evidence but are mode 644. Validation now verifies the manifest against the acquisition receipt and audited digest before raw inspection. Future resumes reject unsafe ownership, modes, file types, containment, parent directories, partials, and quarantine artifacts before body access; no unsafe file can be skipped as verified. Docker uses the trusted system CLI, `--pull=never`, an unmounted version probe, and one read-only raw mount only for validation. Failures expose fixed sanitized categories. The preliminary pinned BIDS Validator 3.0.0/schema 1.2.4 result was not rerun, validation remains incomplete, and the permission-only correction requires separate authorization. No raw permission or file changed, no voxel array was loaded, and preprocessing has not begun.
