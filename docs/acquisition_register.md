...
- **Raw validation status:** Blocked on raw-file permissions. All 22 acquired files remain hash- and size-identical to the acquisition evidence but are mode 644; validation requires private permissions. A preliminary pinned BIDS Validator 3.0.0 run using schema 1.2.4 returned zero errors and 139 warnings (`JSON_KEY_RECOMMENDED`: 3, nonblocking recommendation; `SIDECAR_KEY_RECOMMENDED`: 135, nonblocking recommendation; `README_FILE_MISSING`: 1, expected bounded-subset warning). This is not recorded as completed validation. No raw file was changed, no voxel array was loaded, and preprocessing has not begun.
