...
- **Raw validation status:** Blocked on raw-file permissions. All 22 acquired files remain hash- and size-identical to the acquisition evidence but are mode 644; validation requires private permissions. Default-mode raw partial creation was the acquisition root cause. Future acquisition writes use mode-700 participant-bearing directories and mode-600 partial, final, and quarantine files from creation. The validator is now bound to the exact reviewed container digest with an offline, read-only, least-privilege Docker policy; it permits legal lower-level inheritance overrides and blocks unclassified warnings. A preliminary pinned BIDS Validator 3.0.0 run using schema 1.2.4 returned zero errors and 139 warnings (`JSON_KEY_RECOMMENDED`: 3, nonblocking recommendation; `SIDECAR_KEY_RECOMMENDED`: 135, nonblocking recommendation; `README_FILE_MISSING`: 1, expected bounded-subset warning). It was not rerun for this hardening work and validation is not completed. Applying the permission-only correction requires separate authorization. No raw permission or file was changed, no voxel array was loaded, and preprocessing has not begun.
