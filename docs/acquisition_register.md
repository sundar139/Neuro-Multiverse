...
- **Execution gate:** Execution requires a separate external approval record (mode 600) bound to the exact current `HEAD`, reviewed plan, canonical storage record, and security-critical executor-bundle digest; ancestor approval and command-line approval are rejected. The runtime target must equal the storage record target and remain outside Git with current capacity for the transfer plus reserve. The resolver walks pinned metadata and accepts only exact path/id/size matches under observed OpenNeuro object origins. Subject/path roles, image-sidecar pairs, provider checksums when suitable, strict atomic manifest updates, partial cleanup, promotion recovery, single-run locking, and sanitized common-context events are enforced. A committed non-authorizing template remains `decision: not_approved`; no acquisition occurred, and `independent_approval_verified` and `acquisition_permitted` remain false.
- **Mandatory zero-body preflight:** `--validate-execution` runs the complete execution gate, validates manifest and lock availability without changing them, and resolves all 22 planned objects through exact-origin GraphQL metadata while reporting zero body requests. Ordinary dry-run cannot accept approval or storage evidence. GraphQL redirects remain at the exact metadata endpoint, and event/manifest files are mode 600 from creation. No real approval exists and no transfer occurred.
