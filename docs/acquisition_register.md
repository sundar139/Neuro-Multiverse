...
- **Raw validation status:** Blocked on raw-file permissions. All 22 acquired files remain hash- and size-identical to the acquisition evidence but are mode 644. Validation verifies the manifest against the acquisition receipt and audited digest before raw inspection. Future resumes reject unsafe ownership, modes, file types, containment, parent directories, partials, and quarantine artifacts before body access; no unsafe file can be skipped as verified. The Docker Desktop WSL CLI is accepted only when its root ownership, privileged-group membership, symlink and parent chain, effective non-writability, read-only mount provenance, reviewed SHA-256, and version all match. Mode 775 alone is not a trust decision, and any CLI update requires renewed review. Every Docker command uses pre/post CLI integrity checks, `--pull=never` remains enforced, the version probe has no raw mount, and validation has exactly one read-only raw mount. Failures expose fixed sanitized categories. The preliminary pinned BIDS Validator 3.0.0/schema 1.2.4 result was not rerun, validation remains incomplete, and the permission-only correction requires separate authorization. No raw permission or file changed, no voxel array was loaded, and preprocessing has not begun.
