...
- **Raw validation status:** Blocked on raw-file permissions. All 22 acquired files remain hash- and size-identical to the acquisition evidence but are mode 644. Validation verifies the manifest against the acquisition receipt and audited digest before raw inspection. Future resumes reject unsafe ownership, modes, file types, containment, parent directories, partials, and quarantine artifacts before body access; no unsafe file can be skipped as verified. The reviewed Docker Desktop WSL CLI remains pinned by provenance, version, and SHA-256. Production rejects inherited Docker overrides, uses a minimal credential-free environment, explicitly selects the reviewed local Unix endpoint, and requires the named context and backend identity to agree. CLI and socket identities are checked before and after successful, unsuccessful, and OS-failed commands. Docker Desktop, CLI, context, endpoint, or daemon changes require renewed review. `--pull=never` remains enforced, the version probe has no raw mount, and validation has exactly one read-only raw mount. The real validator was not rerun, validation remains incomplete, and the permission-only correction requires separate authorization. No raw permission or file changed, no voxel array was loaded, and preprocessing has not begun.
