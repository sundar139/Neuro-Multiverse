...
**ds000030 scope.** The planned controlled RQ5 subset is approximately 20 subjects, separate and not selected. The bounded acquisition unit is a pilot of exactly 5 subjects. A metadata-only preflight provisioned the external root, measured capacity, and generated a five-subject plan of 22 files totalling 187,570,603 bytes; the plan and storage-readiness records live outside Git (mode 600) and are referenced only by opaque SHA-256 digests. The external plan carries **no persisted download URL** — runtime URLs are resolved from the pinned OpenNeuro GraphQL snapshot and restricted to the observed dataset object paths. Selected subject identifiers are never committed. Execution requires a separate external **approval record** (mode 600, `decision: approved`) bound to the exact current HEAD, executor bundle, plan, and storage record; a command-line boolean cannot authorize it. The storage target, current mount/device/type, capacity, Ubuntu-24.04 WSL2 identity, and mode-600 execution files must still match. Existing files are verified by strict SHA-256 manifest entries, not size alone; suitable provider checksums are verified while streaming. Atomic promotion/manifest recovery, single-run locking, and sanitized failure events remain external. **No dataset body was downloaded** — the executor runs in `--dry-run` only, which is a real gate (nonzero on any precondition failure). Acquisition remains **unapproved and prohibited** (`independent_approval_verified=false`, `acquisition_permitted=false`); scaling from 5 to ~20 subjects occurs only after pilot acquisition, integrity, BIDS, preprocessing smoke tests, runtime, storage, and governance gates pass.

Before any approved execution, `--validate-execution` must pass as the zero-body final preflight. It shares every execution gate, validates the existing manifest and lock availability without changing them, restricts GraphQL redirects to the exact metadata endpoint, and resolves all 22 planned objects without opening their URLs. Ordinary dry-run rejects supplied approval or storage evidence. External events and manifest files are mode 600 from creation. No real approval exists and no acquisition occurred.
