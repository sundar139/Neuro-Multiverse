...
Raw structural validation remains incomplete. The authorized permission correction completed successfully: exactly 22 acquired raw files are now mode 600 and the 16 required private directories (raw root, five subject, five anat, five func) are mode 700; raw hashes, sizes, ownership, device/inode identities, and mtimes are unchanged from acquisition, and no raw file or directory permission changed during this correction. Permission evidence reference: `ds000030-pilot-permissions-sha256:aeb62b14a73926783543311e3953a1542f2dde5f57cb1b9a7e0216407157680e`. The first post-permission validation attempt stopped before the official BIDS Validator because legitimate public metadata URLs in `dataset_description.json` were misclassified as Windows drive paths by a context-free drive-letter regex over the serialized object; that repository classifier defect was corrected (public HTTP(S) URIs recognized before local-path classification) and subsequently hardened against malformed URL-token boundary cases: nested local schemes, drive paths, UNC paths, invalid hosts, invalid ports, backslashes, credentials, and concatenated URLs are rejected. Private Windows, UNC, POSIX-home, macOS-user, WSL-user, and home-relative paths, file URIs, and credential or signed URIs remain rejected. No raw metadata was rewritten, no data was reacquired, no voxel array was loaded, and preprocessing has not begun. A fresh validation run under the corrected committed validator remains pending separate authorization. ABIDE and COBRE remain blocked, and the approximately 20-subject expansion remains unauthorized.
