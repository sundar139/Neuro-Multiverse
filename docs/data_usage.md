...
Raw structural validation is currently blocked: all 22 acquired files are mode 644, while the read-only validator requires private raw-file permissions. Hashes, sizes, and acquisition evidence still match. The root cause was default-mode raw partial creation in the acquisition executor. Future acquisitions create participant-bearing directories as mode 700 and raw files as mode 600 from creation, then verify those modes through promotion and quarantine. The validator now constructs an offline, read-only Docker invocation bound to the exact reviewed BIDS Validator image digest, accepts legal lower-level BIDS metadata overrides, and blocks unknown warning codes. A preliminary pinned BIDS Validator 3.0.0 run (schema 1.2.4) reported zero errors and 139 recommendation-only warnings, but it was not rerun in this hardening work and the project does not mark raw validation complete. The permission-only correction remains pending separate authorization. No raw permission or file was changed, no voxel array was loaded, and preprocessing has not begun.
