...
Raw structural validation is currently blocked: all 22 acquired files are mode 644, while the read-only validator requires private raw-file permissions. Hashes, sizes, and acquisition evidence still match. The validator binds the supplied manifest digest to both the acquisition receipt and the audited digest before raw inspection. Future acquisition resumes validate file ownership, mode, type, containment, parent directories, partials, and quarantine artifacts before treating any existing file as complete; unsafe files can never be skipped as verified. The Docker Desktop WSL CLI was reviewed by canonical path, root ownership, group membership, symlink and parent-chain replacement resistance, read-only mount provenance, exact version, and SHA-256. Its mode 775 is neither sufficient proof of trust nor of compromise: acceptance depends on effective immutability to the validation user, and a CLI update invalidates the pinned evidence. Every Docker subprocess rechecks the CLI before and after execution; image pulling remains disabled, the version probe mounts no raw data, and validation alone receives one read-only raw mount. Aggregate failures expose only fixed sanitized categories. The preliminary BIDS Validator result was not rerun, the permission-only correction remains pending separate authorization, no raw permission or file changed, and preprocessing has not begun.
