...
Raw structural validation is currently blocked: all 22 acquired files are mode 644, while the read-only validator requires private raw-file permissions. Hashes, sizes, and acquisition evidence still match. The validator now binds the supplied manifest digest to both the acquisition receipt and the audited digest before raw inspection. Future acquisition resumes validate file ownership, mode, type, containment, parent directories, partials, and quarantine artifacts before treating any existing file as complete; unsafe files can never be skipped as verified. Docker is resolved as a trusted system CLI, cannot pull images, and probes the validator version without mounting raw data; validation alone receives one read-only raw mount. Aggregate failures expose only fixed sanitized categories. The preliminary BIDS Validator result was not rerun, the permission-only correction remains pending separate authorization, no raw permission or file changed, and preprocessing has not begun.
